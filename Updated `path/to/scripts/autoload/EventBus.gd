# path/to/scripts/autoload/EventBus.gd
...
# New functions to handle undo and redo events
func emit_undo_event():
    emit_event("undo")

func emit_redo_event():
    emit_event("redo")

# New function to notify about dimension change for physics recalculation
func emit_dimension_change(new_dimension: String):
    emit_event("dimension_changed", new_dimension)
